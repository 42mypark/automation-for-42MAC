#!/bin/bash

USER=mypark
GOINFRE=/Users/$USER/goinfre
CACHE=$GOINFRE/Cache



mkdir -pv $CACHE/.vscode/extensions
mkdir -pv $CACHE/Library/Application\ Support/Code/User/workspaceStorage
mkdir -pv $CACHE/Library/Application\ Support/Code/CachedData
mkdir -pv $CACHE/Library/Application\ Support/Code/CachedExtensionVSIXs
mkdir -pv $CACHE/Library/Application\ Support/Code/CachedExtensions
mkdir -pv $CACHE/Library/Application\ Support/Code/Code\ Cache
mkdir -pv $CACHE/Library/Caches/Homebrew
mkdir -pv $GOINFRE/docker/com.docker.docker
mkdir -pv $GOINFRE/docker/com.docker.helper





